...
const eventMetadata = (eventName: string): Record<string, unknown> => {
  switch (eventName) {
    ...
    case "sign_out":
      return {
        session_duration_ms: faker.number.int({ min: 1000, max: 3_600_000 }),
      };
    case "form_submit":
      return {
        form: faker.word.noun(),
        status: faker.helpers.arrayElement(["success", "error"]),
      };
    case "feature_used":
      return {
        feature: faker.word.noun(),
        duration_ms: faker.number.int({ min: 100, max: 30_000 }),
      };
    case "app_open":
      return {
    ...
    case "purchase":
      return {
        plan: faker.helpers.arrayElement(["pro", "enterprise"]),
        amount_cents: faker.number.int({ min: 999, max: 99900 }),
      };
    case "error":
      return {
...
export const prefillDatabase = async () => {
  try {
    // ── Users ────────────────────────────────────────────────────────────
    const userNames = Array.from({ length: USER_COUNT }, () =>
    ...
    await insertInBatches(
      ...
      (batch) => {
        ...
        return {
          text: `INSERT INTO analytics.user (name) VALUES ${placeholders} ON CONFLICT DO NOTHING`,
          values: batch,
        };
      },
      "users",
    ...
    const { rows: userRows } = await db.query<{ id: number }>(
      `SELECT id FROM analytics.user ORDER BY id`,
    );
    ...
    await insertInBatches(
      ...
      (batch) => {
        ...
        return {
          text: `INSERT INTO analytics.project (name, api_key, user_id) VALUES ${placeholders} ON CONFLICT DO NOTHING`,
          values,
        };
      },
      "projects",
    ...
    // ── Raw events ───────────────────────────────────────────────────────
    const { rows: projectRows } = await db.query<{ id: number }>(
      `SELECT id FROM analytics.project ORDER BY id`,
    );
    ...
    await insertInBatches(
      ...
      (batch) => {
        ...
        return {
          text: `INSERT INTO analytics.raw_event (event_name, metadata, project_id, user_id, timestamp) VALUES ${placeholders} ON CONFLICT DO NOTHING`,
          values,
        };
      },
      "events",
