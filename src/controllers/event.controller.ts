...
import { ASYNC_QUEUE_LIMIT, HTTP_CODE } from "@/utils/constants";
...
const newEvent = async (req: Request, res: Response, _next: NextFunction) => {
    try {
        ...
        if (asyncQueue.length >= ASYNC_QUEUE_LIMIT) {
            logger.error("Queue limit reached to max limit of " + ASYNC_QUEUE_LIMIT)
